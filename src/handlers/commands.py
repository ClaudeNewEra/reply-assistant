...
async def cmd_help(message: Message):
    await message.answer(
        "📖 Как пользоваться\n\n"
        "Отправь мне:\n"
        "1. Текст переписки — скопируй диалог\n"
        "2. Скриншот чата из любого мессенджера\n"
        "3. Голосовое сообщение\n\n"
        "Выбери режим (флирт, дружба, юмор и т.д.) — получи 5 вариантов ответа.\n\n"
        "Команды:\n"
        "/status — сколько анализов осталось\n"
        "/buy — купить пакет анализов\n"
        "/voice — выбрать голос для озвучки ответов"
    )
...
async def cmd_buy(message: Message, bot: Bot):
    ...
    for payload, label, stars, hint in packs:
        await bot.send_invoice(
            ...
            title=f"{label} — {hint}",
            description="AI-анализы переписки с выбором режима",
            payload=payload,
            currency="XTR",
